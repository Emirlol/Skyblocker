...
class EndHudConfigScreen(parent: Screen?) : HudConfigScreen(Text.literal("End HUD Config"), parent, EndHudWidget.Companion.INSTANCE) {
	override fun getConfigPos(config: SkyblockerConfig): List<IntIntMutablePair> {
		return java.util.List.of(IntIntMutablePair.of(config.otherLocations.end.x, config.otherLocations.end.y))
	}
