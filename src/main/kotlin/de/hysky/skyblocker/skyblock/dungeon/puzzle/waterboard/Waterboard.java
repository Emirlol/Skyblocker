...
public class Waterboard extends DungeonPuzzle {
    ...
    public void tick(MinecraftClient client) {
        if (!SkyblockerConfigManager.config.dungeons.puzzleSolvers.solveWaterboard || client.world == null || !DungeonManager.isCurrentRoomMatched() || solve != null && !solve.isDone()) {
            return;
        }
    ...
    public void render(WorldRenderContext context) {
        if (!SkyblockerConfigManager.config.dungeons.puzzleSolvers.solveWaterboard || !DungeonManager.isCurrentRoomMatched()) return;
        Room room = DungeonManager.getCurrentRoom();
    ...
    private ActionResult onUseBlock(PlayerEntity player, World world, Hand hand, BlockHitResult blockHitResult) {
        ...
        if (SkyblockerConfigManager.config.dungeons.puzzleSolvers.solveWaterboard && blockHitResult.getType() == HitResult.Type.BLOCK && waypoints[6] != null && DungeonManager.isCurrentRoomMatched() && blockHitResult.getBlockPos().equals(DungeonManager.getCurrentRoom().relativeToActual(WATER_LEVER)) && state.contains(LeverBlock.POWERED)) {
