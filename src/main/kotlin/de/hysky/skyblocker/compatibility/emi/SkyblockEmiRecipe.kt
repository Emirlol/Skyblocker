...
class SkyblockEmiRecipe(recipe: SkyblockCraftingRecipe) : EmiCraftingRecipe(recipe.getGrid().asSequence().map { stack -> EmiStack.of(stack) }.map { it as EmiIngredient }.toList(), EmiStack.of(recipe.result), Identifier.of("skyblock", getItemId(recipe.result!!).lowercase(Locale.getDefault()).replace(';', '_') + "_" + recipe.result!!.count)) {
	...
	override fun getDisplayHeight(): Int {
		return super.getDisplayHeight() + (if (craftText!!.isEmpty()) 0 else 10)
	}
