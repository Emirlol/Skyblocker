...
class CompactorPreviewTooltipComponent internal constructor(private val items: Iterable<IntObjectPair<ItemStack?>>, private val dimensions: IntIntPair) : TooltipComponent {
	private val columns = max(dimensions.rightInt().toDouble(), 3.0).toInt()
	 ...
	 *
	 *
	 * Draws items on a background of `dimensions.leftInt()` rows and `columns` columns.
	...
	override fun drawItems(textRenderer: TextRenderer, x: Int, y: Int, context: DrawContext) {
		...
		for (entry in items) {
			var itemX = x + entry.leftInt() % dimensions.rightInt() * 18 + 8
			val itemY = y + entry.leftInt() / dimensions.rightInt() * 18 + 18
	...
	companion object {
		...
		private val BLACK_STAINED_GLASS_PANE = ItemStack(Items.BLACK_STAINED_GLASS_PANE)
	}
}
