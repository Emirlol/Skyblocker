...
object ChatRuleAnnouncementScreen {
	...
	fun init() {
		HudRenderEvents.BEFORE_CHAT.register { context: DrawContext, tickDelta: Float ->
			if (timer <= 0 || text == null) return@register
	...
	private fun render(context: DrawContext, tickDelta: Float) {
		val scale = SkyblockerConfigManager.config.chat.chatRuleConfig.announcementScale
		//decrement timer
		timer -= tickDelta
		...
		//render text
		context.drawCenteredTextWithShadow(MinecraftClient.getInstance().textRenderer, text, 0, 0, -0x1)
	...
	fun setText(newText: Text) {
		text = newText
		timer = SkyblockerConfigManager.config.chat.chatRuleConfig.announcementLength.toFloat()
	}
}
